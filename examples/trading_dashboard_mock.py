#!/usr/bin/env python
"""
Mock Trading Dashboard Example with WebSockets

This example demonstrates how to create a trading dashboard that uses
WebSockets for real-time data streaming with realistic mock market data.
The dashboard displays realistic market data and trading signals in real-time.

Run this example with:
    python examples/trading_dashboard_mock.py
"""
...
# Current realistic market prices (as of May 2025)
current_prices = {
    "BTC-USDT": 103757.0,
    "ETH-USDT": 7250.0,
    "SOL-USDT": 320.0
...
async def simulate_market_data():
    ...
    for _ in range(30):
        for symbol in symbols:
            # Realistic price fluctuations (0.1% - 0.2%)
            price_change = current_prices[symbol] * (0.001 + 0.001 * random.random()) * (1 if random.random() > 0.5 else -1)
    ...
    while True:
        for symbol in symbols:
            # Realistic price fluctuations (0.05% - 0.15%)
            price_change = current_prices[symbol] * (0.0005 + 0.001 * random.random()) * (1 if random.random() > 0.5 else -1)
            current_prices[symbol] += price_change
            ...
            signal = strategies[symbol].update(current_prices[symbol])
            if signal:
                logger.info(f"Generated {signal['signal']} signal for {symbol} at {current_prices[symbol]:.2f}")
...
def generate_dashboard_html():
    ...
    html_path = examples_dir / "mock_dashboard.html"
...
<body>
    <div class="container">
        <div class="header">
            <h1>Trading Dashboard (Realistic Mock Data)</h1>
...
async def main():
    ...
    print("Starting Trading Dashboard Example with Realistic Mock Data...")
    ...
    # Start the WebSocket server on a different port (8766)
...
if __name__ == "__main__":
    try:
        ...
        print("\nExiting Trading Dashboard Example...")
    except Exception as e:
        logger.error(f"Error in main: {str(e)}")
        print(f"\nError: {str(e)}")
        print("Exiting Trading Dashboard Example...")
    finally:
        print("Goodbye!")
        sys.exit(0)
